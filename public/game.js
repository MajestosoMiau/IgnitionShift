let isLoggedIn = false; // This will track the login state
let currentSection = 'home'; // Default to 'home' if not set

// Wait for the DOM to load before executing scripts
document.addEventListener('DOMContentLoaded', () => {
  // Navigate to the 'home' section on initial load
window.onload = function() {
  navigateTo('home');
};
  // Initialize other logic
  updatePlayerGold();  // Ensure this is called after initializing the player
  
  // Add event listener for chat button
  const chatButton = document.getElementById('chatButton');
  chatButton.addEventListener('click', toggleChat);  

  // Other DOM interactions like displaying messages
...
function purchaseItem(item) {
    if (item.dualCurrencyRequirement) {
        ...
        if (player.gold >= goldCost && player.diamonds >= diamondCost) {
            player.gold -= goldCost;
            player.diamonds -= diamondCost;
            player.ownedItems.push(item);
            alert(`You purchased a ${item.name} for ${goldCost} Gold and ${diamondCost} Diamonds!`);

            // Hide the car section and reset view
            hideCarSection();
            resetShopView();
            ...
            alert(`You need ${goldCost} Gold and ${diamondCost} Diamonds to purchase this item.`);
        }
    } else {
        const currency = item.currency;
        const balance = currency === 'diamonds' ? player.diamonds : player.gold;
...
// Shop Items with descriptions and adjusted stats
const shopItems = {
  cars: [
    {
      name: 'Rusty Rider',
      image: "images/rusty_rider.jpg",
      stats: {
          speed: 3,
          power: 4,
          dexterity: -1,
      ...
      description: 'A reliable, albeit old, model.',
      price: 1000
    },
    {
      name: 'Dirt Devil',
      image: 'images/dirt_devil.jpg',
      stats: {
          speed: 12,
          power: 6,
          dexterity: 7,
    ...
    {
      name: 'Mountain Mule',
      image: 'images/mountain_mule.jpg',
      stats: {
          speed: 4,
          power: 7,
          dexterity: 2,
      ...
      description: 'Boost your performance with this high-output engine.',
      dualCurrencyRequirement: {
        goldCost: 2000,
        diamondCost: 200
    }},
    {
      name: 'Desert Stormer',
      image: 'images/desert_stormer.jpg',
      stats: {
          speed: 10,
          power: 6,
          dexterity: 4,
      ...
      description: 'Built for extreme desert terrains.',
      price: 4200
    },
    {
      name: 'Flash Racer',
      image: 'images/flash_racer.jpg',
      stats: {
          speed: 15,
          power: 7,
          dexterity: 6,
      ...
      description: 'An incredibly fast and sleek racer.',
      price: 4750
    },    
    {
      name: 'Royal Cruiser',
      image: 'images/royal_cruiser.jpg',
      stats: {
          speed: 6,
          power: 5,
          dexterity: 2,
    ...
    {
      name: 'Velocity Viper',
      image: 'images/velocity_viper.jpg',
      stats: {
          speed: 18,
          power: 9,
          dexterity: 8,
      ...
      description: 'Lightning fast with cutting-edge technology.',
      price: 10000
    },  
],
  engines: [
      {name: 'Raggedy Engine',
        image: 'images/raggedy_engine',
        stats: {
            speed: 1,
            power: 3,
            dexterity: 0,
        ...
        description: 'A bad engine, but will get you from A to B',
        price: 700
      },
      { name: 'Ultra Blaster',
        image: 'images/ultra_blaser',
        stats: {
            speed: 43,
            power: 37,
            dexterity: -12,
        ...
        description: 'This massive beast will make you soil yourself at its sound',
        price: 50000
      },
        ],
  tires: [
      { name: 'Sport Tires', handling: 5, luck: -1, price: 100, description: 'Designed for optimal grip on the road, but reduces luck slightly.' },
      { name: 'All-Weather Tires', handling: 4, luck: 0, price: 80, description: 'Reliable tires suitable for any weather conditions.' }
  ],
  suspensions: [
      { name: "Sport Suspension", handling: 6, structure: 2, luck: -1, price: 200, description: 'Enhance handling but trade-off some luck for performance.' },
      { name: "Heavy-Duty Suspension", handling: 4, structure: 5, luck: 0, price: 300, description: 'Sturdy suspension perfect for rough terrains.' }
  ],
  turbos: [
      { name: "Basic Turbo", power: 2, speed: 1, luck: 0, price: 150, description: 'Get a boost in speed and power for your vehicle.' },
      { name: "Advanced Turbo", power: 3, speed: 2, luck: -1, price: 250, description: 'High performance turbocharger with a slight luck decrease.' }
  ],
  paintworks: [
      { name: "Metallic Paint", luck: 1, price: 100, description: 'Give your car a sleek look and a touch of luck.' },
      { name: "Glossy Paint", luck: 2, price: 150, description: 'Make your car shine brighter while enhancing luck.' }
  ],
  seats: [
      { name: "Comfort Seat", structure: 1, dexterity: 1, luck: 0, price: 120, description: 'Experience maximum comfort during your rides.' },
      { name: "Racing Seat", structure: 2, dexterity: 2, luck: -1, price: 180, description: 'Race-ready seat that improves dexterity but may reduce comfort.' }
  ]
...
// Define the function to show car items
function displayCarItems() {
  ...
  const carHTML = shopItems.cars.map(item => `
    <div class="car-item">
      <img src="${item.image}" alt="${item.name}" style="width: 150px; height: auto;" />
      <h3>${item.name}</h3>
